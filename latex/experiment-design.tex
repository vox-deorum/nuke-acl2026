...
Appendix~\ref{app:reproduction-details} provides reproduction details, including dataset and code links. We conduct a $2\times2\times2$ factorial experiment to surface potential mechanisms behind LLMs' nuclear authorization behaviors. Each intervention modifies less than 1\% (avg. <500 tokens) of avg. $\sim$50,000 tokens per turn in a typical game state. Figure~\ref{fig:game-state-components} summarizes the game-state and output components inherited from Vox Deorum and CivBench.
...
\textbf{Prompt context} & System/developer instructions, prior-turn rationale, and replay intervention text. \\
\textbf{Game state} & Victory progress, diplomacy, economy, military conditions, cities, units, technologies, policies, and recent events. \\
\textbf{Strategist output} & Pre-decision reasoning trail, structured tool calls, flavor changes including \texttt{use-nuke}, and post-hoc rationale. \\
\textbf{Execution layer} & Non-LLM tactical modules carry out selected macro-strategic settings in Civilization V. \\
\end{tabular}
\end{minipage}}
\caption{Main game-state and output components used in replay. The schematic abstracts the Vox Deorum strategic interface and CivBench replay records; see \citet{chen2025voxdeorum} and \citet{chen2026civbench} for infrastructure details.}
\label{fig:game-state-components}
\end{figure}
...
To analyze the reasoning trails, we construct two \textbf{validated keyword concepts} by selecting from the word-stem frequency list and validating through human-AI deductive coding on 200 positive and 200 negative trails, randomly sampled from the full corpus (Appendix~\ref{app:keyword-concept-validation}).
