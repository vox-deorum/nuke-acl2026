...
\begin{itemize}
\item \textbf{Condition main-effects regression.} OLS with regressors \texttt{ethical}, \texttt{no\_rationale}, \texttt{high\_stakes}, extended with two-way condition interactions and \texttt{model} $\times$ \texttt{condition} terms. The same form fits separately within each model to identify model-specific outliers (Appendix~\ref{app:per-model-condition-coefficients}).
