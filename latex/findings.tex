...
Among the 880 coded trails with explicit ethical keywords (excluding Gemini-3.5-Flash and MiniMax-M2.7), 11 models differed not only in whether they took up the ethical prompt, but in how they incorporated it, a factor associated with different de-escalation outcomes. Considering the prompt as a directive to follow (13.6\%) or constraint to consider (67.4\%) are both de-escalation factors ($\beta$ = -29.67***; -13.97**), while merely acknowledging the prompt does not help ($\beta_{\mathrm{ind}}$ = +31.84***; full codebook in Appendix~\ref{app:deductive-codebook}, regression tables in Appendix~\ref{app:deductive-code-regressions}).
