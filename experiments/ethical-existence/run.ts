...
const load = new LoadJsonStep({
    path: "./workspaces/nuke-2026/ethical-existence",
});
...
const coder1 = new CodeStep({
    ...
    codebook: "./workspaces/nuke-2026/ethical-existence/codebook.json",
    parameters: { 
        temperature: 0.5
    },
});
...
const consolidate = new ConsolidateStep({
    coder: [coder1, coder2, coder3, ensemble],
    builderConfig: {
        consolidators: [],
    },
});
...
const config: QAJobConfig = {
    steps: [load, coder1, coder2, coder3, ensemble, consolidate, reliability],
    parallel: true,
};
