...
[fig: keyword appearance, explicitly ethical]
For most models, nuke-specific ethical prompting increases ethical reasoning keywords together with a game-framing defense. Ethical keywords appear almost only in ethical conditions with a widely ranged induced rate: Kimi-K2.6 reaches 75%, Qwen-3.6-27B sits around 10%, while MiniMax-M2.7 does not react. Such appearance is strongly associated with the ethical intervention's effect. Under the cluster-bootstrapped probe, adding the ethical-keyword indicator attenuates the majority of the ethical prompting's impact in all comparison pairs (62-88% on average), with per model differences [Appendix]. Game or simulation keywords also increase for most models (avg. 2.3% => 12.1%), likely to defend the escalation (see Finding 3) [Appendix]. 
